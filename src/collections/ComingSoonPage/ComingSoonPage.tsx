...
import {
  ...
  developmentStatus,
  listingType,
} from '../Condos/Variables/DefaultFilter';
  ...
  return (
    <Box marginTop={'30px'}>
            ...
            {!isMobile && (
              ...
              >
                ...
                <CitySearchableFilter
                  value={filter.city}
                  cities={cities}
                  onChange={({ label }) => handleFilterChange('city', label)}
                  fullWidth={false}
                />
              </Stack>
            )}
            <Divider styles={{ marginBlock: '30px' }} />
            {fetching && filter.page === 1 ? (
              <Stack
                alignItems={'center'}
                justifyContent={'center'}
                height='400px'
      ...
      <Box
        ...
        ref={ref}
      >
        {fetching && filter.page !== 1 && state.fetchMore && (
          <CircularProgress />
        )}
      </Box>
    </Box>
